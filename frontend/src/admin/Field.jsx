...
export const inputStyle = {
  ...
  fontFamily: "var(--font-body)", fontSize: "0.9375rem", boxSizing: "border-box",
};

export function Section({ title, children }) {
  return (
    <div style={{ marginBottom: "2rem" }}>
      <h2 style={{ fontSize: "0.875rem", textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-muted)", margin: "0 0 1rem", fontWeight: 500 }}>{title}</h2>
...
export function Row({ label, children }) {
  ...
  return (
    <div style={isMobile
      ...
      <label style={{ fontSize: "0.875rem", color: "var(--text-muted)", display: "block", ...(isMobile ? { marginBottom: "0.25rem" } : { paddingTop: "0.5rem" }) }}>{label}</label>
      <div>{children}</div>
    </div>
  );
}
